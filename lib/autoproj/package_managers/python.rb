    ...
    module Python
        ...
        def self.get_python_version(python_bin)
            ...
            if status.success?
                ...
                raise RuntimeError, "Autoproj::Python.get_python_version identification"\
        ...
        def self.get_pip_version(pip_bin)
            ...
            if status.success?
                ...
                raise RuntimeError, "Autoproj::Python.get_pip_version identification"\
        ...
        def self.find_python(ws: Autoproj.workspace,
            ...
            raise RuntimeError, "Autoproj::Python.find_python_bin: failed to find python" \
                " for version '#{version}'"
        end
        ...
        def self.custom_resolve_python(ws: Autoproj.workspace,
            ...
            if valid
                ...
                raise RuntimeError, "Autoproj::Python.resolve_python: requested python"\
        ...
        def self.auto_resolve_python(ws: Autoproj.workspace,
            ...
            resolvers.each do |resolver|
                begin
                    ...
                    Autoproj.debug "Autoproj::Python.resolve_python: resolver failed: #{e}"
                end
            end

            if !bin
                msg = "Autoproj::Python.resolve_python: failed to find a python executable"
                if version_constraint
        ...
        def self.prepare_venv(ws: Autoproj.workspace, bin: nil, version: nil,
          ...
          python_bin,_ = resolve_python(ws: ws, bin: bin, version: version)
          cmd = "#{python_bin} -m pip install --user -U virtualenv"
          msg, status = Open3.capture2e(cmd)
          if not status.success?
              raise RuntimeError, "Autoproj::Python.prepare_venv installation of "\
          ...
          msg, status = Open3.capture2e(cmd)
          if not status.success?
              raise RuntimeError, "Autoproj::Python.prepare_venv preparation of"\
        ...
        def self.setup_python_configuration_options(ws: Autoproj.workspace)
            ...
            if ws.config.get("USE_PYTHON")
                ...
                ws.config.declare 'USE_PYTHON_VENV', 'boolean',
                    default: 'no',
                    doc: [ "Do you want to use a virtual environment for python?" ]
