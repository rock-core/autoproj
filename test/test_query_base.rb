    ...
    describe QueryBase do
        ...
        describe ".parse" do
            it "parses FIELD=VALUE as an exact match" do
                query = QueryBase.parse('test.field=20', allowed_fields: ['test.field'])
                assert_equal ['test', 'field'], query.fields
            ...
            it "raises if the string contains neither = nor ~" do
                e = assert_raises(ArgumentError) { QueryBase.parse('20') }
            ...
            it "raises if the FIELD is not in the list of allowed fields" do
                e = assert_raises(ArgumentError) { QueryBase.parse('test.field=20') }
                assert_equal "'test.field' is not a known query key", e.message
            end
            it "maps FIELD using the default fields first" do
                query = QueryBase.parse('unknown~20', allowed_fields: ['test.field'], default_fields: { 'unknown' => 'test.field' })
                assert_equal ['test', 'field'], query.fields
                assert_equal '20', query.value
                assert query.partial?
            end
        ...
        describe ".parse_query" do
            before do
                @query_class = Class.new(QueryBase) do
                    def self.parse(str)
                        super(str, allowed_fields: ['test.field'])
                    end
                end
            end
            ...
            it "combines multiple queries using the And" do
                ...
                sub = query.each_submatch.to_a
                assert_equal 2, sub.size

                q0 = sub[0]
                assert_equal ['test', 'field'], q0.fields
                assert_equal '20', q0.value
                refute q0.partial?

                q1 = sub[1]
                assert_equal ['test', 'field'], q1.fields
