...
module Autoproj
    describe OSPackageResolver do
        ...
        def setup
            super
            @operating_system = [%w[test debian default], ["v1.0", "v1", "default"]]
        end
        ...
        def test_supported_operating_system
            ...
            resolver = OSPackageResolver.new
            resolver.operating_system = [%w[test debian default], ["v1.0", "v1", "default"]]
            assert(resolver.supported_operating_system?)
            resolver.operating_system = [%w[test default], ["v1.0", "v1", "default"]]
            assert(!resolver.supported_operating_system?)
        end
        ...
        describe "handling of global entries" do
            ...
            it "returns the global entries even if there are no OS-specific ones" do
                ...
                expected = [[osdeps.os_package_manager, FOUND_PACKAGES, %w[global_pkg1 global_pkg2]]]
                assert_equal expected, osdeps.resolve_package("pkg")
            end
            ...
            it "marks the package as nonexistent if an OS-specific version entry marks it explicitely so" do
                ...
                expected = [[osdeps.os_package_manager, FOUND_NONEXISTENT, %w[global_pkg1 global_pkg2]]]
                assert_equal expected, osdeps.resolve_package("pkg")
            end
        ...
        def test_resolve_os_version_global_and_specific_nonexistent
            ...
            expected = [[osdeps.os_package_manager, FOUND_NONEXISTENT, %w[global_pkg1 global_pkg2 pkg0 pkg1]]]
            assert_equal expected, osdeps.resolve_package("pkg")
        end
        ...
        def test_resolve_os_version_global_and_specific_ignore
            ...
            expected = [[osdeps.os_package_manager, FOUND_PACKAGES, %w[global_pkg1 global_pkg2 pkg0 pkg1]]]
            assert_equal expected, osdeps.resolve_package("pkg")
        end
        ...
        def test_resolve_osindep_packages_global
            ...
            data = { "gem" => %w[gempkg gempkg1] }
            osdeps = create_osdep(data)
            expected = [["gem", FOUND_PACKAGES, %w[gempkg gempkg1]]]
            assert_equal expected, osdeps.resolve_package("pkg")
        ...
        def test_resolve_osindep_packages_specific
            ...
            osdeps = create_osdep(data)
            expected = [["gem", FOUND_PACKAGES, %w[pkg gempkg]]]
            assert_equal expected, osdeps.resolve_package("pkg")
        end
